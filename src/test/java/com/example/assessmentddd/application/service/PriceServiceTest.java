...
class PriceServiceTest {
    ...
    void getPrice_found_returnsResponse() {
        ...
        when(pricePort.findByApplicationDateProductAndBrand(date, 35455, 1))
                .thenReturn(java.util.Optional.of(testPrice));
        ...
        verify(pricePort).findByApplicationDateProductAndBrand(date, 35455, 1);
    }

    @Test
    void getPrice_notFound_throwsPriceNotFoundException() {
        LocalDateTime date = LocalDateTime.parse("2020-01-01T10:00:00");
        when(pricePort.findByApplicationDateProductAndBrand(date, 99999, 999))
        ...
        verify(pricePort).findByApplicationDateProductAndBrand(date, 99999, 999);
    }
    ...
    void getPrice_callsPortOnce() {
        ...
        LocalDateTime date = LocalDateTime.parse("2020-06-14T10:00:00");
        when(pricePort.findByApplicationDateProductAndBrand(any(), anyInt(), anyInt()))
                .thenReturn(java.util.Optional.of(testPrice));

        priceService.getPrice(date, 35455, 1);

        verify(pricePort, times(1)).findByApplicationDateProductAndBrand(date, 35455, 1);
